...
const buildPlaylistVideos = (data) => {
  const items = get__default['default'](data, "contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content.sectionListRenderer.contents[0].itemSectionRenderer.contents[0].playlistVideoListRenderer.contents");
  return items.reduce((acc, item) => {
    ...
    return [
      ...
      {
        id,
        name: get__default['default'](body, "title.runs[0].text", ""),
...
const buildPlaylist = (data) => {
  const maybeTitle = get__default['default'](data, "metadata.playlistMetadataRenderer.title", "");
  return {
...
async function getPlaylistHtml(playlistId, fetchOptions) {
  ...
  const response = await fetch__default['default'](getPlaylistUrl(playlistId), options);
...
//# sourceMappingURL=youtube-playlist-scraper.js.map
